...
<div class='panel panel-<{$panel_type}>'>
    ...
    <div class='panel-body'>
        <{if $list}>
					...
					<{if $attribute.value || $attribute.info}>    
						<div id="aorder_<{$attribute.id}>" order="<{$attribute.weight}>" class="row wgre-obj-detail-row col-xs-12 col-sm-12">
							...
							<{if $attribute.attdef_type == $smarty.const.WGREALESTATE_ATTR_TEXT_KWH_VAL}>
								<div class='clear'></div>
								<div class='col-xs-12 col-sm-12'><i class='glyphicon glyphicon-arrow-down' style='padding-left:<{$attribute.kwh_left}>%'></i></div>
					...
					<{if $attribute.value || $attribute.info}>    
						...
						<{/if}>
							...
							<div class='wgre-obj-detail-right col-xs-12 col-sm-4'>
								<{if $attribute.value_user}><p><{$attribute.value_user}></p><{/if}>
								<{if $attribute.value_user_yes}><p><img class='wgre-img-yes' src='<{$wgrealestate_icon_url_32}>yes.png' alt='<{$attribute.value_user}>' title='<{$attribute.value_user}>'  alt='<{$attribute.value_user}>'></p><{/if}>
								<{if $attribute.info}><p><{$attribute.info_user}></p><{/if}>
							</div>
							<{if $attribute.attdef_type == $smarty.const.WGREALESTATE_ATTR_TEXT_KWH_VAL}>
								<div class='clear'></div>
								<div class='col-xs-12 col-sm-6'><i class='glyphicon glyphicon-arrow-down' style='padding-left:<{$attribute.kwh_left}>%'></i></div>
								<div class='clear'></div>
						...
						<div class='row wgre-img-gallery'>
							<ul class='pgwSlideshow'>
								<{foreach item=image from=$images}>
									<li><img src='<{$wgrealestate_obj_image_url}>medium/<{$image.name}>' alt='<{$image.title}>' title='<{$image.title}>' data-description='<{$image.info}>'></li>
								<{/foreach}>
							</ul>
						</div>
					<{else}>
						<div class='wgre-img-obj col-xs-12 col-sm-12 center'>
							<{if $object.mainimage}>
								<img class='img-responsive wgre-img-main' src='<{$wgrealestate_obj_image_url}>/medium/<{$object.mainimage}>' title='<{$object.name}>' alt='<{$object.name}>'>
							<{/if}> 
						</div>
					<{/if}> 
				</div>
						...
						<div class='row wgre-img-gallery'>
							<ul class='pgwSlideshowPlan'>
								<{foreach item=planimage from=$planimages}>
									<li><img src='<{$wgrealestate_obj_image_url}>medium/<{$planimage.name}>' alt='<{$planimage.title}>' title='<{$planimage.title}>' data-description='<{$planimage.info}>'></li>
								<{/foreach}>
							</ul>
						</div>
					<{else}>
			...
			<{if $object.map_location}>
				<div id="map" class="row wgre-obj-detail-row object-map col-xs-12 col-sm-12">
					<div class='wgre-obj-header-left col-xs-12 col-sm-12'>
					<iframe
						width="100%"
						height="450"
						frameborder="0" style="border:0"
						<{if $object.obj_address}>
							src="https://www.google.com/maps/embed/v1/place?key=<{$googlemapsapikey}>&q=place_id:<{$object.obj_geo_placeid}>" allowfullscreen>
						<{else}>
							src="https://www.google.com/maps/embed/v1/search?key=<{$googlemapsapikey}>&q=<{$map_filter}><{$object.map_location}>" allowfullscreen>
						<{/if}>
					</iframe>
					</div>
					<!--
					<div class='wgre-obj-header-right right col-xs-1 col-sm-1'>
						<a class='wgre-btn center' href='objects.php?op=<{$op}>&amp;obj_id=<{$object.id}>&map_filter=restaurants' title='<{$smarty.const._MA_WGREALESTATE_RESTAURANTS}>'>
